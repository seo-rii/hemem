...
#define DRAM_SIZE (8L * (1024L * 1024L * 1024L))
#define NVM_SIZE (525L * (1024L * 1024L * 1024L))
...
static void *do_gups(void *arguments)
{
  ...
  uint64_t hotbit = log2(args->hotsize) - 1;
...
static void *map_memory(void *arguments) {
  ...
  uint64_t page = 0;
  for(; page * HUGE_PAGE_SIZE < (1 << hotbit); ++page) {
    void *newptr = mmap((void*)&field[page * stride], HUGE_PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE | MAP_FIXED, dramfd, dram_offset);
    dram_size -= HUGE_PAGE_SIZE;
    dram_offset += HUGE_PAGE_SIZE;
    if (newptr == MAP_FAILED) {
      fprintf(stderr, "line %d: %ld %ld %ld\n", __LINE__, page, page * stride, dram_offset);
      perror("newptr mmap  ");
      assert(0);
    }
  ...
  stride = BASE_PAGE_SIZE / sizeof(uint64_t);
  // Map each page to appropriate location
  for(; page < size / BASE_PAGE_SIZE; ++page) {
    void *newptr;
    if(page % sparsity == 0 && page / sparsity < hot_pages && dram_size >= BASE_PAGE_SIZE) {
      ...
      newptr = mmap((void*)&field[page * stride], BASE_PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE | MAP_FIXED, nvmfd, nvm_offset);
      nvm_size -= BASE_PAGE_SIZE;
      nvm_offset += BASE_PAGE_SIZE;
      if(nvm_size <= 0) {
...
int main(int argc, char **argv)
{
  ...
  fprintf(stderr, "Soft limit %ld, hard %ld\n", rlim.rlim_cur, rlim.rlim_max);
